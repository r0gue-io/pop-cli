...
impl Zombienet {
            ...
            .and_then(|p| p.as_array_of_tables())
        {
            for table in tables.iter() {
                let id = table
                    ...
                    .ok_or(anyhow!("expected `parachain` to have `id`"))?
                    as u32;
    ...
    pub async fn spawn(&mut self) -> Result<Network<LocalFileSystem>> {
        ...
        let relay_chain_config = network_config
            .get_mut("relaychain")
        ...
        if let Some(tables) = network_config
            .get_mut("parachains")
        ...
        for file in ["polkadot-execute-worker", "polkadot-prepare-worker"] {
            ...
            symlink_file(
                self.cache
                    .join(format!("{file}-{}", self.relay_chain.version)),
                dest,
            )?;
        }
