...
1. **All work happens on `main`** — commits, PRs, feature branches all merge to `main`.
2. **Never reference `master` in code or docs** — if you see `master` anywhere, it's a bug that needs fixing.
3. **The `master` branch must stay synchronized with `main`** — after pushing to `main`, also push to `master`:
