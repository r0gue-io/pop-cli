...
pub fn next_block() {
	...
	// pallet_template::on_initialize(System::block_number());
	#[cfg(feature = "try-runtime")]
	pallet_template::try_state(System::block_number()).expect("State corrupted");
}
...
pub fn run_to_block(n: BlockNumberFor<Test>) {
	while System::block_number() < n {
		if System::block_number() > 1 {
			...
			// pallet_template::on_finalize(System::block_number());
		}
		next_block();
	}
}
    ...
    ($test_name: ident, $extrinsic: ident, $($arg: expr),*) =>{
        ...
        fn $test_name(){
            new_test_ext().execute_with(||{
                assert_noop!(pallet_template::$extrinsic(RuntimeOrigin::none(), $($arg),*), DispatchError::BadOrigin);
            })
        }
    };
}
