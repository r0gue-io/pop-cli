...
use crate::{errors::Error, utils::helpers::canonicalized_path, ContractTemplate};
use anyhow::Result;
...
pub fn create_smart_contract(name: &str, target: &Path, template: &ContractTemplate) -> Result<()> {
...
pub fn rename_contract(name: &str, path: PathBuf, template: &ContractTemplate) -> Result<()> {
...
mod tests {
	...
	fn setup_test_environment(template: ContractTemplate) -> Result<tempfile::TempDir, Error> {
	...
	fn test_create_standard_smart_contract_success() -> Result<(), Error> {
		let temp_dir = setup_test_environment(ContractTemplate::Standard)?;
	...
	fn test_create_template_smart_contract_success() -> Result<(), Error> {
		let temp_dir = setup_test_environment(ContractTemplate::ERC20)?;
		// Verify that the generated smart contract contains the expected content
		let generated_file_content =
			fs::read_to_string(temp_dir.path().join("test_contract/lib.rs"))
				.expect("Could not read file");
	...
	fn generate_testing_files_and_folders(
		template: ContractTemplate,
	...
	fn test_extract_contract_files() -> Result<(), Error> {
		// ERC-20
		let mut temp_dir = generate_testing_files_and_folders(ContractTemplate::ERC20)?;
		let mut output_dir = tempfile::tempdir()?;
		extract_contract_files(
			ContractTemplate::ERC20.to_string(),
			temp_dir.path(),
			output_dir.path(),
		)?;
		...
		// ERC-721
		temp_dir = generate_testing_files_and_folders(ContractTemplate::ERC721)?;
		output_dir = tempfile::tempdir()?;
		extract_contract_files(
			ContractTemplate::ERC721.to_string(),
			temp_dir.path(),
			output_dir.path(),
		)?;
		assert!(output_dir.path().join("lib.rs").exists());
		...
		assert!(!output_dir.path().join("noise_folder").exists());
		// ERC-1155
		temp_dir = generate_testing_files_and_folders(ContractTemplate::ERC1155)?;
		output_dir = tempfile::tempdir()?;
		extract_contract_files(
			ContractTemplate::ERC1155.to_string(),
			temp_dir.path(),
			output_dir.path(),
		)?;
		assert!(output_dir.path().join("lib.rs").exists());
	...
	fn test_rename_contract() -> Result<(), Error> {
		...
		rename_contract("my_contract", temp_dir.path().to_owned(), &ContractTemplate::ERC20)?;
		let generated_cargo =
