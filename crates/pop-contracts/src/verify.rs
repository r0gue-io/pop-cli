...
fn get_build_info_parsed_from_contract_bundle(
	...
	if let Some(info) = metadata.source.build_info {
		...
		Ok(BuildInfoParsed { build_info, build_mode, polkavm_blob })
	} else {
...
fn compare_local_toolchain(build_info: BuildInfo) -> Result<(), Error> {
...
fn verify_polkavm_blob_against_build_result(
	polkavm_blob: &[u8],
