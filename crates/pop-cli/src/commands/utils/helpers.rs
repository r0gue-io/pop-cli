...
/// A macro to facilitate the selection of an options among some pre-selected ones (represented as variants from a fieldless enums) and the naming of that selection. This process is repetead until the user stops it and then the result is stored in a `Vec` of tuples, containing the options and the given names, which is returned at the end of the macro expansion. This is useful in processes such as designing a pallet's storage to generate a pallet template: we need the user to pick among the different storage types (StorageValue, StorageMap, StorageDoubleMap,...) and give names to those storages.
///
/// # Parameters
///
...
/// - `$prompt_message`: The messages displayed to the user at the very beginning of each iteration. They must implement the `Dispay` trait. Each enum must have associated a prompt message.
/// # Note
...
/// # Note
/// This macro only works with a 1-byte sized enums, this is, fieldless enums with at most 255 elements each. This is because we're just interested in letting the user to pick some options among a predefined set, then the name should be descriptive enough, and 1-byte sized enums are really easy to convert to and from a `u8`, so we can work with `u8` all the time and just recover the variant at the end.
///
/// The decision of using 1-byte enums instead of just fieldless enums is for simplicity: we won't probably offer a user to pick from > 256 options. If this macro is used with enums containing fields, the conversion to `u8` will simply be detected at compile time and the compilation will fail. If this macro is used with fieldless enums greater than 1-byte (really weird but possible), the conversion to u8 will overflow and lead to unexpected behavior, so we panic at runtime if that happens for completeness.
///
/// # Example
...
macro_rules! multiselect_pick{
    ($enum: ty, $prompt_message: expr) =>{
        {
            // Ensure the enum is 1-byte long. This is needed cause fieldless enums with > 256 elements will lead to unexpected behavior as the conversion to u8 for them isn't detected as wrong at compile time. Enums containing variants with fields will be catched at compile time. Weird but possible. 
            assert!(std::mem::size_of::<$enum>() == 1);
            let mut prompt = multiselect(format!("{} {}", $prompt_message, "Pick an option by pressing the spacebar. Press enter when you're done!")).required(false);
            ...
            // The unsafe block is safe cause the bytes are the discriminants of the enum picked above, qed; 
            prompt.interact()?.iter().map(|byte| unsafe{std::mem::transmute(*byte)}).collect::<Vec<TemplatePalletConfigCommonTypes>>()
        }
    }
}
