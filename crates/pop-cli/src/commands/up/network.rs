...
impl<const FILTER: u8> BuildCommand<FILTER> {
	...
	pub(crate) async fn execute(
		...
		let mut rollups = self.parachain.clone();
		...
		let network_config =
			NetworkConfiguration::build(relay.clone(), self.port, rollups.as_deref())?;

		let parachain_names: Option<Vec<_>> =
			rollups.as_ref().map(|p| p.iter().map(|r| r.name().to_string()).collect());
