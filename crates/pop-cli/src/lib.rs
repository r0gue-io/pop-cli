...
use pop_telemetry::{Telemetry, config_file_path, record_cli_used};
use std::{
...
impl Cli {
	...
	pub async fn execute(&mut self) -> Result<()> {
		self.command.execute().await.map(|_| ())
	}

	/// Command name used for telemetry.
	#[cfg(feature = "telemetry")]
	pub fn event_name(&self) -> String {
		self.command.to_string()
	}

	/// Full serialized command payload used for telemetry.
	#[cfg(feature = "telemetry")]
	pub fn command_payload(&self) -> serde_json::Value {
		serde_json::json!(self.command)
	}
}
