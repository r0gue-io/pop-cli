...
/// # Output
/// Option<PahBuf>: Some if the input is contained in a workspace, the inner PathBuf points to the workspace Cargo.toml. Otherwise, None
pub(crate) fn find_workspace_toml(target_dir: &Path) -> Option<PathBuf> {
...
/// # Errors
/// - crate_path isn't inside the workspace directory
/// - member isn't an array
/// - workspace isn't present in the workspace Cargo.toml
pub(crate) fn add_crate_to_workspace(
	workspace_toml: &Path,
	crate_path: &Path,
) -> anyhow::Result<()> {
