...
pub fn test_project(path: &Path, maybe_test_filter: Option<String>) -> Result<(), Error> {
...
mod tests {
	...
	fn test_project_works() -> Result<(), Error> {
		...
		test_project(&temp_dir.path().join("test_contract"), None)?;
		Ok(())
	}

	#[test]
	fn test_project_with_filter_works() -> Result<(), Error> {
		let temp_dir = tempfile::tempdir()?;
	...
	fn test_project_wrong_directory() -> Result<(), Error> {
		let temp_dir = tempfile::tempdir()?;
		assert!(matches!(
			test_project(&temp_dir.path().join(""), None),
